...
const SearchResult = ({ ingredientButtons }) => {
  ...
  const allButtons = buttons.map((ingredientBtn, i) => {
    ...
    return (
      <div key={i} className="button-search-parent">
        <Button
          className="button-background-color button-default"
          onClick={() => dispatch(openIngredient(ingredientBtn))}
        >
          {ingredientBtn.strMeal}
        </Button>
      </div>
    );
  });
  ...
  return (
    <div>
      {allButtons == null ? (
        <div data-testid="search-result-buttons"></div>
      ) : (
        <div
          data-testid="search-result-buttons"
